			...
			conn.query(sql, [event.thingid], function (err, result) {
				if (err) {
					...
					for(var i=0; i<result.length; i++) {
						var comment = {commentid:result[i]['commentid'],commenttext:result[i]['commenttext'], commentposted:result[i]['commentposted'], commentuserid:result[i]['commentuserid'],commentthingid:result[i]['commmentthingid'], commentattachmentid:result[i]['commentattachmentid']};
