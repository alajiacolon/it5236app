...
class Application {
    ...
    public function auditlog($context, $message, $priority = 0, $userid = NULL){
		...
		$url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/AuditLog";
			$data = array(
				'context'=>$context,
				'message'=>$message,
				'logdate'=>$logdate,
        ...
        $ch = curl_init();
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: X7mFNepgfE70ewlbTwrZ8aUDNHSSA29I3oaqCp5a'));
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/registeruser";
			$data = array(
				'userid'=>$userid,
				'username'=>$username,
   ...
   */
    ...
    protected function sendValidationEmail($userid, $email, &$errors) {
		...
		$url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/sendValidationEmail";
			$data = array(
        ...
        if ($response === FALSE) {
            ...
            $message = "A request has been made to create an account at http://54.236.49.104/it5236/website for this email address. ".
            ...
            $headers = 'From: webmaster@54.236.49.104' . "\r\n" .
                'Reply-To: webmaster@54.236.49.104' . "\r\n";
    ...
    public function newSession($userid, &$errors, $registrationcode = NULL) {
        ...
        if (sizeof($errors) == 0) {
			...
			$url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/newSession";
			$data = array(
    ...
    public function getUserRegistrations($userid, &$errors) {
		...
		$url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/getUserRegistration?userid=" . $userid;
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: X7mFNepgfE70ewlbTwrZ8aUDNHSSA29I3oaqCp5a'));
    ...
    public function updateUserPassword($userid, $password, &$errors) {
        ...
        if(sizeof($errors) == 0) {
            ...
            $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/updateUserPassword";
             $data = array(
               'userid'=>$userid,
               'passwordHash'=>$passwordhash
             );
    ...
    protected function clearPasswordResetRecords($passwordresetid) {
      ...
      $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/clearPasswordResetRecords";
      $data = array(
        ...
        if (sizeof($errors) == 0) {
            ...
            if ($result === FALSE) {
                ...
                if (!password_verify($password, $row['passwordhash'])) {
                    ...
                    //make a connection to database to insert the bind2hexin
            					$dbh = $this->getConnection();
            					$email = $row['email'];
            					$username = $row['userid'];
            					$hexLogin = bin2hex(random_bytes(3));
            					$sql = "INSERT INTO validatelogin (username, login_hex, email, time)" .
            						"VALUES (:username, :loginhex, :email, NOW())";

            					$stmt = $dbh->prepare($sql);
            					$stmt->bindParam(":email", $email);
            					$stmt->bindParam(":username", $username);
            					$stmt->bindParam(":loginhex", $hexLogin);
            					$result = $stmt->execute();
            					if ($result === FALSE){
            						$errors[] = "An unexpected error occurred while sending the validation email";
            						$this->debug($stmt->errorInfo());
            						$this->auditlog("register error", $stmt->errorInfo());
            					}else {
 ...
 /*public function otp($otp) {
   // Validate the user input
   if (empty($otp)) {
       $errors[] = "Missing One Time Password";
   }
   ...
   if (sizeof($errors) == 0) {
       ...
       // Construct a SQL statement to perform the insert operation
       $sql = "SELECT login_hex FROM validelogin " .
           "WHERE username = :username";
    ...
    public function logout() {
        ...
        if (!empty($sessionid)) {
            ...
            $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/logout?usersessionid=" . $sessionid;
            // Connect to the database
            $ch = curl_init();
        		curl_setopt($ch, CURLOPT_URL, $url);
        		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    ...
    public function getThings(&$errors) {
        ...
        $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/getThings?thingregistrationcode=". $registrationcode;
        ...
        if ($response === FALSE) {
          ...
          if($httpCode == 400) {
            ...
            // Get all the rows
            $decode = json_decode($response)->things;
           foreach($decode as $key){
                $key = array(
                  'thingid'=>$key->thingid,
                  'thingcreated'=>$key->thingcreated,
                  'thingattachmentid'=>$key->attachmentid,
                  'registrationcode'=>$key->registrationcode,
                  'thingname'=>$key->thingname
                );
                $things[] = $key;
              }
            $this->auditlog("getThings", "Succesfull Add thing");
          }
    ...
    public function getThing($thingid, &$errors) {
        ...
        if (sizeof($errors) == 0){
          ...
          $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/getThing?thingid=" . $thingid;
           $ch = curl_init();
           curl_setopt($ch, CURLOPT_URL, $url);
               curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: X7mFNepgfE70ewlbTwrZ8aUDNHSSA29I3oaqCp5a'));
           curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
           ...
           $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            ...
            if ($result === FALSE) {
              ...
              if($httpCode == 400) {
                 ...
                 $thing = array(
                   'thingid'=>$decode->thingid,
                   'thingname'=>$decode->thingname,
                   'thingcreated'=>$decode->thingcreated,
                   'thinguserid'=>$decode->thinguserid,
                   'thingattachmentid'=>$decode->thingattachmentid,
                   'filename'=>$decode->filename,
                   'thingregistrationcode'=>$decode->thingregistrationcode
                 );
               }//end else if
               curl_close($ch);
             }//end else
    ...
    public function getComments($thingid, &$errors) {
        ...
        if (empty($thingid)) {
          ...
          $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/getComment?thingid=" . $thingid;
          $ch = curl_init();
          curl_setopt($ch, CURLOPT_URL, $url);
              curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: X7mFNepgfE70ewlbTwrZ8aUDNHSSA29I3oaqCp5a'));
          curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
          ...
          $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            ...
            if ($result === FALSE) {
              ...
              if($httpCode == 400) {
                 ...
                 $decode = json_decode($response)->list;
                 foreach($decode as $key){
                      $key = array(
                        'commentthingid'=>$decode->commentthingid,
                        'commentid'=>$decode->commentid,
                        'commenttext'=>$decode->commenttext,
                        'commentposted'=>$decode->commentposted,
    ...
    public function addThing($name, $attachment, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/addThing";
            // Create a new ID
    ...
    public function addComment($text, $thingid, $attachment, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            if (sizeof($errors) == 0) {
                ...
                // Construct a SQL statement to perform the insert operation
                $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/addComment";
                $data = array(
                  'commentid'=>$commentid,
                  'commenttext'=>$text,
                  'userid'=>$userid,
        ...
        if(sizeof($errors)== 0) {
            ...
            if (!$isadmin && $loggedinuserid != $userid) {
                ...
                if (sizeof($errors) == 0) {
                  ...
                  $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/getUser?userid=" . $userid;
                 $ch = curl_init();
                 curl_setopt($ch, CURLOPT_URL, $url);
                     curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: X7mFNepgfE70ewlbTwrZ8aUDNHSSA29I3oaqCp5a'));
                 curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    ...
    public function newAttachmentType($name, $extension, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            $url = "https://szqz52yeh9.execute-api.us-east-1.amazonaws.com/default/newAttachmentType";
            $data = array(
            ...
            if ($response === FALSE) {
                ...
                if($httpCode == 400) {
                  ...
                  $this->auditlog("newAttachmentType Added", $name);
                }
            }
            curl_close($ch);
        } else {
